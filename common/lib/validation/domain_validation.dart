...
class DomainValidator {
  ...
  static final List<String> EMPTY_STRING_ARRAY = [];

  // Regular expression strings for hostnames (derived from RFC2396 and RFC 1123)

  // RFC2396: domainlabel   = alphanum | alphanum *( alphanum | "-" ) alphanum
  // Max 63 characters
  static final String DOMAIN_LABEL_REGEX = '\\p{Alnum}(?>[\\p{Alnum}-]{0,61}\\p{Alnum})?';

  // RFC2396 toplabel = alpha | alpha *( alphanum | "-" ) alphanum
  // Max 63 characters
  static final String TOP_LABEL_REGEX = '\\p{Alpha}(?>[\\p{Alnum}-]{0,61}\\p{Alnum})?';
  ...
  static final String DOMAIN_NAME_REGEX = '^(?:' + DOMAIN_LABEL_REGEX + '\\.)+' + '(' + TOP_LABEL_REGEX + ')\\.?\$';

  static final String UNEXPECTED_ENUM_VALUE = 'Unexpected enum value: ';

  bool allowLocal;

  /// Singleton instance of this validator, which
  ///  doesn't consider local addresses as valid.
  static final DomainValidator DOMAIN_VALIDATOR = DomainValidator.allow(false);

  /// Singleton instance of this validator, which does
  ///  consider local addresses valid.
  static final DomainValidator DOMAIN_VALIDATOR_WITH_LOCAL = DomainValidator.allow(true);
  ...
  final RegexValidator domainRegex = RegexValidator.regex(DOMAIN_NAME_REGEX);

  /// RegexValidator for matching a local hostname
  // RFC1123 sec 2.1 allows hostnames to start with a digit
  final RegexValidator hostnameRegex = RegexValidator.regex(DOMAIN_LABEL_REGEX);
  ...
  /// @return the singleton instance of this validator
  static DomainValidator getInstanceWithAllow(bool allowLocal) {
    inUse = true;
    if (allowLocal) {
  ...
  /// {@link #updateTLDOverride(ArrayType, List<String>)} method
  ...
  /// @since 1.7
  static DomainValidator getInstanceWithItems(bool allowLocal, List<Item> items) {
    inUse = true;
    return DomainValidator.withItems(allowLocal, items);
  }

  // intance variables allowing local overrides
  List<String> mycountryCodeTLDsMinus;
  List<String> mycountryCodeTLDsPlus;
  List<String> mygenericTLDsPlus;
  List<String> mygenericTLDsMinus;
  List<String> mylocalTLDsPlus;
  List<String> mylocalTLDsMinus;
  /*
     * N.B. It is vital that instances are immutable.
     * This is because the default instances are shared.
    */

  // N.B. The constructors are deliberately to avoid possible problems with unsafe publication.
  ...
  DomainValidator.allow(bool allowLocal) {
  ...
  DomainValidator.withItems(bool allowLocal, List<Item> items) {
    this.allowLocal = allowLocal;

    // default to class overrides
    var ccMinus = countryCodeTLDsMinus;
    var ccPlus = countryCodeTLDsPlus;
    var genMinus = genericTLDsMinus;
    var genPlus = genericTLDsPlus;
    var localMinus = localTLDsMinus;
    var localPlus = localTLDsPlus;
    ...
    for (final item in items) {
      ...
      for (var i = 0; i < item.values.length; i++) {
        copy[i] = item.values[i].toLowerCase();
      }
      copy.sort();
      switch (item.type) {
        case ArrayType.COUNTRY_CODE_MINUS:
          {
            ccMinus = copy;
            break;
          }
        case ArrayType.COUNTRY_CODE_PLUS:
          {
            ccPlus = copy;
            break;
          }
        case ArrayType.GENERIC_MINUS:
          {
            genMinus = copy;
            break;
          }
        case ArrayType.GENERIC_PLUS:
          {
            genPlus = copy;
            break;
          }
        case ArrayType.LOCAL_MINUS:
          {
            localMinus = copy;
            break;
          }
        case ArrayType.LOCAL_PLUS:
          {
            localPlus = copy;
            break;
          }
  ...
  bool isValid(String domain) {
    ...
    var groups = domainRegex.match(domain);
  ...
  bool isValidDomainSyntax(String domain) {
    ...
    var groups = domainRegex.match(domain);
    return (groups != null && groups.isNotEmpty) || hostnameRegex.isValid(domain);
  }
  ...
  bool isValidInfrastructureTld(String iTld) {
    final key = chompLeadingDot(unicodeToASCII(iTld).toLowerCase());
  ...
  bool isValidGenericTld(String gTld) {
    final key = chompLeadingDot(unicodeToASCII(gTld).toLowerCase());
  ...
  bool isValidCountryCodeTld(String ccTld) {
    final key = chompLeadingDot(unicodeToASCII(ccTld).toLowerCase());
  ...
  bool isValidLocalTld(String lTld) {
    final key = chompLeadingDot(unicodeToASCII(lTld).toLowerCase());
    return (arrayContains(LOCAL_TLDS, key) || arrayContains(mylocalTLDsPlus, key)) && !arrayContains(mylocalTLDsMinus, key);
  }
  ...
  String chompLeadingDot(String str) {
    if (str.startsWith('.')) {
  ...
  static final List<String> INFRASTRUCTURE_TLDS = <String>[
    'arpa', // internet infrastructure
  ];

  // WARNING: this array MUST be sorted, otherwise it cannot be searched reliably using binary search
  static final List<String> GENERIC_TLDS = <String>[
...
//        "aigo", // aigo aigo Digital Technology Co,Ltd. [Not assigned as of Jul 25]
    ...
    'bananarepublic', // bananareThe Gap, Inc.
...
//        "zippo", // zippo Zadco Company
  ...
  // WARNING: this array MUST be sorted, otherwise it cannot be searched reliably using binary search
  static final List<String> COUNTRY_CODE_TLDS = <String>[
...
//        "an",                 // Netherlands Antilles (retired)
    ...
    'cd', // Democratic Reof the Congo (formerly Zaire)
...
//        "tp",                 // East Timor (Retired)
  ...
  // WARNING: this array MUST be sorted, otherwise it cannot be searched reliably using binary search
  static final List<String> LOCAL_TLDS = <String>[
  ...
  /*
     ...
     */
  // WARNING: this array MUST be sorted, otherwise it cannot be searched reliably using binary search
  static List<String> countryCodeTLDsPlus = EMPTY_STRING_ARRAY;

  // WARNING: this array MUST be sorted, otherwise it cannot be searched reliably using binary search
  static List<String> genericTLDsPlus = EMPTY_STRING_ARRAY;

  // WARNING: this array MUST be sorted, otherwise it cannot be searched reliably using binary search
  static List<String> countryCodeTLDsMinus = EMPTY_STRING_ARRAY;

  // WARNING: this array MUST be sorted, otherwise it cannot be searched reliably using binary search
  static List<String> genericTLDsMinus = EMPTY_STRING_ARRAY;

  // WARNING: this array MUST be sorted, otherwise it cannot be searched reliably using binary search
  static List<String> localTLDsMinus = EMPTY_STRING_ARRAY;

  // WARNING: this array MUST be sorted, otherwise it cannot be searched reliably using binary search
  static List<String> localTLDsPlus = EMPTY_STRING_ARRAY;
  ...
  /// For example:
  /// <p>
  /// {@code DomainValidator.updateTLDOverride(ArrayType.GENERIC_PLUS, new List<String>{"apache"})}
  /// <p>
  ...
  static void updateTLDOverride(ArrayType table, List<String> tlds) {
    if (inUse) {
      throw ('Can only invoke this method before calling getInstance');
    }
    final copy = <String>[];
    // Comparisons are always done with lower-case entries
    for (var i = 0; i < tlds.length; i++) {
      copy[i] = tlds[i].toLowerCase();
    }
    copy.sort();
    switch (table) {
      case ArrayType.COUNTRY_CODE_MINUS:
      ...
      case ArrayType.LOCAL_PLUS:
        localTLDsPlus = copy;
        break;
      case ArrayType.COUNTRY_CODE_RO:
      case ArrayType.GENERIC_RO:
      case ArrayType.INFRASTRUCTURE_RO:
      case ArrayType.LOCAL_RO:
        throw ('Cannot update the table: $table');
      default:
        throw (UNEXPECTED_ENUM_VALUE + '$table');
    }
  }
  ...
  /// @since 1.5.1
  static List<String> getTLDEntries(ArrayType table) {
    List<String> array;
    switch (table) {
      case ArrayType.COUNTRY_CODE_MINUS:
  ...
  /// @since 1.7
  List<String> getOverrides(ArrayType table) {
    List<String> array;
    switch (table) {
      case ArrayType.COUNTRY_CODE_MINUS:
  ...
  static String unicodeToASCII(String input) {
    ...
    try {
      ...
      var lastChar = input[length - 1]; // fetch original last char
      switch (lastChar) {
        case '\u002E': // "." full stop
        case '\u3002': // ideographic full stop
        case '\uFF0E': // fullwidth full stop
        case '\uFF61': // halfwidth ideographic full stop
          return String.fromCharCodes(ascii) + '.'; // restore the missing stop
        default:
          return String.fromCharCodes(ascii);
      }
    } catch (e) {
  ...
  /*
     ...
     */
  static bool isOnlyASCII(String input) {
    if (input == null) {
      return true;
    }
    for (var i = 0; i < input.length; i++) {
      if (input[i].codeUnits[0] > 0x7F) {
  ...
  static bool arrayContains(List<String> sortedArray, String key) {
...
class IDNBUGHOLDER {
  static bool keepsTrailingDot() {
    final input = 'a.'; // must be a valid name
    return input.codeUnits == (input.codeUnits);
  }

  static final bool IDN_TOASCII_PRESERVES_TRAILING_DOTS = keepsTrailingDot();
}
...
class Item {
  ...
  Item(ArrayType type, List<String> values) {
    this.type = type;
    this.values = values; // no need to copy here
  }
}

/// enum used by {@link DomainValidator#updateTLDOverride(ArrayType, List<String>)}
/// to determine which override array to update / fetch
/// @since 1.5.0
/// @since 1.5.1 made and added read-only array references
enum ArrayType {
