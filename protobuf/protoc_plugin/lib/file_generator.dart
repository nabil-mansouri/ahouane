...
const String _protobufImport = "import 'package:protobuf/protobuf.dart' as $_protobufImportPrefix;";
...
const String _typedDataImport = "import 'dart:typed_data' as $_typedDataImportPrefix;";
const String _convertImport = "import 'dart:convert' as $_convertImportPrefix;";
const String _grpcImport = "import 'package:grpc/service_api.dart' as $_grpcImportPrefix;";
...
class FileGenerator extends ProtobufContainer {
  ...
  static Map<String, PbMixin> _getDeclaredMixins(FileDescriptorProto desc) {
    ...
    final importedMixins = desc.options.getExtension(Dart_options.imports) as Imports;
    ...
    PbMixin resolveMixin(String name) {
      ...
      if (dartMixins.containsKey(name)) {
        ...
        var pbMixin = PbMixin(dartMixin.name, importFrom: dartMixin.importFrom, parent: resolveMixin(dartMixin.parent));
  ...
  /// style.
  final Set<String> usedTopLevelNames = <String>{}..addAll(forbiddenTopLevelNames);
  ...
  final Set<String> usedExtensionNames = <String>{}..addAll(forbiddenExtensionNames);

  /// True if cross-references have been resolved.
  bool _linked = false;

  FileGenerator(this.descriptor, this.options) : protoFileUri = Uri.file(descriptor.name) {
    ...
    var defaultMixinName = descriptor.options?.getExtension(Dart_options.defaultMixin) as String ?? '';
    var defaultMixin = declaredMixins[defaultMixinName] ?? findMixin(defaultMixinName);
    ...
    for (var i = 0; i < descriptor.enumType.length; i++) {
      enumGenerators.add(EnumGenerator.topLevel(descriptor.enumType[i], this, usedTopLevelNames, i));
    }
    for (var i = 0; i < descriptor.messageType.length; i++) {
      messageGenerators.add(MessageGenerator.topLevel(descriptor.messageType[i], this, declaredMixins, defaultMixin, usedTopLevelNames, i));
    }
    for (var i = 0; i < descriptor.extension.length; i++) {
      extensionGenerators.add(ExtensionGenerator.topLevel(descriptor.extension[i], this, usedExtensionNames, i));
    }
    for (var service in descriptor.service) {
      if (options.useGrpc) {
        grpcGenerators.add(GrpcServiceGenerator(service, this));
      } else {
        var serviceGen = ServiceGenerator(service, this, usedTopLevelServiceNames);
        serviceGenerators.add(serviceGen);
        clientApiGenerators.add(ClientApiGenerator(serviceGen, usedTopLevelNames));
      }
    }
  }
  ...
  List<CodeGeneratorResponse_File> generateFiles(OutputConfiguration config) {
    ...
    if (options.generateMetadata) {
      files.addAll([
        makeFile(".pb.dart.meta", mainWriter.sourceLocationInfo.writeToJson().toString()),
        makeFile(".pbenum.dart.meta", enumWriter.sourceLocationInfo.writeToJson().toString())
      ]);
    }
    if (options.useGrpc) {
    ...
    files.add(
      makeFile(
        ".pboptions.dart",
        generateMessageOptionFile(
          config: config,
          linked: _linked,
          makeWriter: makeWriter,
          protoFileUri: protoFileUri,
          writeHeading: _writeHeading,
          writeImport: _writeImport,
          fileGenerator: this,
          messages: descriptor.messageType,
  ...
  IndentingWriter makeWriter() => IndentingWriter(filename: options.generateMetadata ? descriptor.name : null);

  /// Returns the contents of the .pb.dart file for this .proto file.
  IndentingWriter generateMainFile([OutputConfiguration config = const DefaultOutputConfiguration()]) {
    if (!_linked) throw StateError("not linked");
    var out = makeWriter();
  ...
  void writeMainHeader(IndentingWriter out, [OutputConfiguration config = const DefaultOutputConfiguration()]) {
    _writeHeading(out);
    ...
    if (_needsFixnumImport) {
      out.println("import 'package:fixnum/fixnum.dart' as $_fixnumImportPrefix;");
    }
    ...
    for (var publicDependency in descriptor.publicDependency) {
      _writeExport(out, config, Uri.file(descriptor.dependency[publicDependency]), '.pb.dart');
    }

    // Export enums in main file for backward compatibility.
    if (enumCount > 0) {
      var resolvedImport = config.resolveImport(protoFileUri, protoFileUri, ".pbenum.dart");
  ...
  bool get _needsProtobufImport => messageGenerators.isNotEmpty || extensionGenerators.isNotEmpty || clientApiGenerators.isNotEmpty;

  /// Returns the generator for each .pb.dart file we need to import.
  void _findProtosToImport(Set<FileGenerator> imports, Set<FileGenerator> enumImports) {
    for (var m in messageGenerators) {
  ...
  List<String> findMixinImports() {
    ...
    return mixins.map((mixin) => mixin.importFrom).toSet().toList(growable: false)..sort();
  }

  /// Returns the contents of the .pbenum.dart file for this .proto file.
  IndentingWriter generateEnumFile([OutputConfiguration config = const DefaultOutputConfiguration()]) {
    if (!_linked) throw StateError("not linked");
  ...
  String generateServerFile([OutputConfiguration config = const DefaultOutputConfiguration()]) {
    if (!_linked) throw StateError("not linked");
    var out = makeWriter();
    _writeHeading(out, extraIgnores: {'deprecated_member_use_from_same_package'});
  ...
  String generateGrpcFile([OutputConfiguration config = const DefaultOutputConfiguration()]) {
    if (!_linked) throw StateError("not linked");
    var out = makeWriter();
    _writeHeading(out);
  ...
  void writeBinaryDescriptor(IndentingWriter out, String identifierName, String name, GeneratedMessage descriptor) {
  ...
  String generateJsonFile([OutputConfiguration config = const DefaultOutputConfiguration()]) {
    if (!_linked) throw StateError("not linked");
    var out = makeWriter();
    _writeHeading(out, extraIgnores: {'deprecated_member_use_from_same_package'});
    ...
    for (var e in enumGenerators) {
      ...
      writeBinaryDescriptor(out, e.binaryDescriptorName, e._descriptor.name, e._descriptor);
    }
    for (var m in messageGenerators) {
      m.generateConstants(out);
      writeBinaryDescriptor(out, m.binaryDescriptorName, m._descriptor.name, m._descriptor);
    }
    for (var s in serviceGenerators) {
      s.generateConstants(out);
      writeBinaryDescriptor(out, s.binaryDescriptorName, s._descriptor.name, s._descriptor);
    }
  ...
  void _writeHeading(IndentingWriter out, {Set<String> extraIgnores = const <String>{}}) {
    var extraIgnoresString = extraIgnores.isEmpty ? '' : ',${extraIgnores.join(',')}';
    out.println('''
///
...
''');
  ...
  void _writeImport(IndentingWriter out, OutputConfiguration config, FileGenerator target, String extension) {
    var resolvedImport = config.resolveImport(target.protoFileUri, protoFileUri, extension);
  ...
  /// (Possibly the same .proto file.)
  void _writeExport(IndentingWriter out, OutputConfiguration config, Uri target, String extension) {
