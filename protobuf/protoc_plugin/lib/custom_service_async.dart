...
final asyncGenerators = <ServiceAsyncGenerator>[];
String generateServiceAsyncFile({
  ...
  final void Function(IndentingWriter out, OutputConfiguration config,
  ...
  for (final service in services) {
    asyncGenerators
        .add(ServiceAsyncGenerator(service, fileGenerator)..resolve(context));
  }
  ...
  for (var generator in asyncGenerators) {
...
class ServiceAsyncGenerator {
  ...
  final _methods = <_AsyncMethod>[];

  ServiceAsyncGenerator(this._descriptor, this.fileGen) {
    ...
    _serviceClassname =
        name.endsWith('Service') ? name + 'Async' : name + 'ServiceAsync';
  }
  ...
  void resolve(GenerationContext ctx) {
    for (var method in _descriptor.method) {
      _methods.add(_AsyncMethod(this, ctx, method));
    }
  }
  ...
  void _generateService(IndentingWriter out) {
    out.addBlock('abstract class $_serviceClassname extends $_service {', '}',
      ...
      out.println('$_coreImportPrefix.bool get \$isAsync => true;');
      out.println();
...
class _AsyncMethod {
  ...
  factory _AsyncMethod(ServiceAsyncGenerator service, GenerationContext ctx,
  ...
  void generateServiceMethodPreamble(IndentingWriter out) {
    ...
    out.addBlock(
        ...
        '}', () {
      if (_serverStreaming) {
        out.println('yield* $_dartName(await request);');
      } else {
        out.println('return $_dartName(await request);');
      }
    });
    out.println();
  }
