...
typedef FrozenMessageErrorHandler = void Function(String messageName, [String methodName]);

void defaultFrozenMessageModificationHandler(String messageName, [String methodName]) {
  if (methodName != null) {
    throw UnsupportedError('Attempted to call $methodName on a read-only message ($messageName)');
  }
  throw UnsupportedError('Attempted to change a read-only message ($messageName)');
}
...
FrozenMessageErrorHandler _frozenMessageModificationHandler = defaultFrozenMessageModificationHandler;
FrozenMessageErrorHandler get frozenMessageModificationHandler => _frozenMessageModificationHandler;
...
class _FieldSet {
  ...
  Map<K, V> _getDefaultMap<K, V>(MapFieldInfo<K, V> fi) {
    ...
    if (_isReadOnly) {
      return PbMap<K, V>.unmodifiable(PbMap<K, V>(fi.keyFieldType, fi.valueFieldType, fi.mapEntryBuilderInfo));
    }
  ...
  void _setField(int tagNumber, value) {
    ...
    if (fi.isRepeated) {
      throw ArgumentError(_setFieldFailedMessage(fi, value, 'repeating field (use get + .add())'));
    }
  ...
  int get _hashCode {
    ...
    int hashEachField(int hash) {
      ...
      hash = _infosSortedByTag
          .where((fi) => _values[fi.index] != null)
          .fold(hash, (int h, FieldInfo fi) => hashField(h, fi, _values[fi.index]));

      if (!_hasExtensions) return hash;

      hash = _sorted(_extensions._tagNumbers).fold(hash, (int h, int tagNumber) {
        var fi = _extensions._getInfoOrNull(tagNumber);
  ...
  void writeString(StringBuffer out, String indent) {
    ...
    _infosSortedByTag.forEach((FieldInfo fi) => writeFieldValue(_values[fi.index], fi.name));

    if (_hasExtensions) {
      _extensions._info.keys.toList()
        ..sort()
        ..forEach((int tagNumber) => writeFieldValue(_extensions._values[tagNumber], '[${_extensions._info[tagNumber].name}]'));
    }
    if (_hasUnknownFields) {
  ...
  void _mergeField(FieldInfo otherFi, fieldValue, {bool isExtension}) {
    ...
    if (otherFi.isGroupOrMessage) {
      final currentFi = isExtension ? _ensureExtensions()._getFieldOrNull(fi) : _values[fi.index];
  ...
  void _shallowCopyValues(_FieldSet original) {
    ...
    for (var index = 0; index < _meta.byIndex.length; index++) {
      ...
      if (fieldInfo.isMapField) {
        ...
        if (map != null) {
          _values[index] = (fieldInfo as MapFieldInfo)._createMapField(_message)..addAll(map);
        }
      } else if (fieldInfo.isRepeated) {
        PbListBase list = _values[index];
        if (list != null) {
          _values[index] = fieldInfo._createRepeatedField(_message)..addAll(list);
        }
      }
    }
